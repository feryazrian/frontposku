<template>
  <div class="pos-navbar-top" v-if="isUserLogin">
    <div class="pos-navbar-top-left">
      ...
      <navtop-search :disableStatus="disable_status"></navtop-search>

      <div class="pos-nav-top-product">
        <span
          class="custom_img"
          :title="$t('pos_home.navtop.add_custom_product_tooltip')"
          @click="showCustomModal('addCustomProduct')"
        ></span>
      </div>
    </div>
    ...
    <div class="pos-navbar-top-right">
      <div
        class="nav-reload"
        :title="$t('pos_home.navtop.reset_data_tooltip')"
        @click="reloadPosData"
      >
      ...
      <div class="nav-wifi online_label">
        <i
          class="fa fa-wifi"
          :title="$t('pos_home.navtop.offline_mode_tooltip')"
          @click="offlineManage(true)"
        ></i>
      </div>
      ...
      <div class="nav-top-user" v-if="pos_user.id">
        <div class="user-image">
          <img
            v-if="pos_user.image"
            :src="pos_user.img_path"
            :alt="pos_user.firstname + ' ' + pos_user.lastname"
            :title="pos_user.firstname + ' ' + pos_user.lastname"
          />
        ...
        <div class="user-logout">
          <div :title="$t('pos_home.navtop.logout_tooltip')" @click="userLogout(pos_user.id)">
...
export default {
  ...
  methods: {
    ...
    userLogout(user_id) {
      ...
      //     this.$store.dispatch(setApp.CUSTOMER.CURRENT_DEFAULT)
