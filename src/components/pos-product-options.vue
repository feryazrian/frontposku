<template>
    <div class="pos-attributes">
        ...
        <div class="pos-buttons text-center">
            <button type="button" :disabled="disabled_button" class="pos-button button-md button-dark" @click="validateOptions(childAttributes)">{{ $t('pos_home.pos_products.btn_done') }}</button>
        </div>
    </div>
</template>
...
<script>
    ...
    export default {
        ...
        created () {
            ...
            // var configObj = JSON.parse(this.product_variations);
            // configObj = JSON.parse(this.product_variations);
        ...
        methods: {
            validateOptions(attributes) {
                ...
                if ( attr_length != selected_attr_length ) {
                    ...
                    let make_converted_price = selected_price.split(window.pos_currency_symbol);

                    if (make_converted_price.length > 1 && make_converted_price[1]) {
                        converted_price = parseFloat(make_converted_price[1]);
                    }
            ...
            getAttributeOptions (attributeId) {
                ...
                this.config.attributes.forEach(function(attribute) {
