          ...
          <div class="pos-customer-fields">
            <div class="control-group" :class="[this.errors.has('discount') ? 'has-error' : '']">
              ...
              >
                ...
                <option
                  :value="discount.id"
                  :data-name="discount.offername"
                  v-for="(discount, index) in pos_discounts"
                  :key="index"
                >{{ discount.offername }}</option>
              </select>
              <span
                class="control-error"
                v-if="this.errors.has('discount')"
              >{{ this.errors.first('discount') }}</span>
            </div>

            <div v-if="discount_details_status" class="pos-setting-list">
              <div class="pos-setting row-layout">
                <div class="setting-list-name">
                  <div class="name">{{ discount_details.offername }}</div>
                </div>
                <div class="setting-list-rate">
                  <span
                    v-if="discount_details.type == 'percentage'"
                  >{{ discount_details.converted_value }}%</span>
                  <span v-else>{{ discount_details.fvalue }}</span>
                </div>
                <div
                  class="setting-list-range"
                >Range {{ discount_details.ffromprice }} to {{ discount_details.ftoprice }}</div>
              </div>
            </div>
            ...
            <div class="pos-action text-center">
              <button
                ...
                type="submit"
                :text="$t('pos_home.pos_cart.button_apply_discount')"
              >{{ $t('pos_home.pos_cart.button_apply_discount') }}</button>
            </div>
          </div>
        </div>
      </div>
        ...
        if (thisthis.pos_offline) {
          if (Object.keys(thisthis.localObject.pos_discounts).length > 0) {
            if (thisthis.discount) {
              $.each(
                thisthis.localObject.pos_discounts,
                (key, discount_detail) => {
                  if (discount_detail.id === thisthis.discount) {
                    thisthis.discount_details = discount_detail;
    ...
    applyDiscount(discountDetails) {
      if (discountDetails) {
        ...
        EventBus.$emit("setLocalForage", {
          key: "pos_discount",
          data: JSON.stringify(discountDetails)
        });

        this.$toast.success(
          "Success: Discount applied to the current cart successfully!",
          {
            position: "top-right",
            duration: 2000
          }
