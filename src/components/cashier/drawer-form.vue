        ...
        <div class="form-container">
          <div class="pos-customer-fields">
            <div class="control-group" :class="[this.errors.has('amount') ? 'has-error' : '']">
              ...
              <span
                class="control-error"
                v-if="this.errors.has('amount')"
              >{{ this.errors.first('amount') }}</span>
            </div>
            ...
            <div class="pos-action text-center">
              <button
                ...
                type="submit"
                :text="$t('pos_cashier.menu_content.button_open_store')"
              >{{ $t('pos_cashier.menu_content.button_open_store') }}</button>
            </div>
          </div>
        </div>
      </div>
...
<script>
import { openDrawer } from "@/api/drawer";
...
export default {
  ...
  methods: {
    ...
    drawerAction() {
      ...
      // thisthis.formAction = "/api/pos/auth/openDrawer";
          ...
          openDrawer({
            ...
            .then(response => {
              if (!response.data.status) {
                this.$toast.error(response.data.message, {position: "top-right", duration: 2000});
                ...
                this.$root.toggleButtonDisable(false);
                // this.$root.hideCommonModal("addDrawer");
                this.closeCashDrawer("addDrawer");
              }
            })
            .catch(e => {
              this.$toast.error('opss, something wrong!', {position: "top-right", duration: 2000});
              this.$root.toggleButtonDisable(false);
              console.error(e)
            })
        } 
