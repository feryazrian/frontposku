  ...
  <div class="pos-content">
    <div class="cart" v-if="getCartTotal">
      <div class="cart-header">
        <div class="cart-hold-section">
          <span>{{ $t("pos_home.pos_cart.cart_details") }}</span>
        ...
        <div class="cart-count-section">
          <div
            v-if="showMinusBtn"
            class="btn btn-sm btn-pos-primary cart-btn"
            v-bind:selctedcart="pos_current_cart"
            :title="$t('pos_home.pos_cart.delete_current_cart')"
            @click="removeCart(false)"
          >
          ...
          <ul class="pos-nav-lists">
            <li
              v-for="(cart, index) in pos_carts"
              :key="index"
              :class="{ 'pos-nav': true, 'active': index == pos_current_cart }"
              :label="'cart_count_' + index"
              @click="activeCartNav(index)"
            >{{ index + 1 }}</li>
          </ul>
      ...
      <div class="pos-nav-content">
        <div
          v-for="(cart, index) in pos_carts"
          :key="cart.id"
          :class="{ 'pos-nav-pane': true, 'active': index == pos_current_cart }"
          :id="'cart_count_' + index"
        >
          <ul class="cart_details">
            ...
            <div v-else>
              <li v-for="(cart_detail, i) in cart" :key="i" :class="{'active': cart_detail.active}">
                <div class="cart-product-content">
                  ...
                  <div class="product-qty">
                    <span>
                      ...
                      - {{ parseFloat(cart_detail.price).toLocaleString(getLocale) }} {{ $t("pos_home.pos_cart.text_per_unit") }}
                    </span>
                  </div>
                </div>
                <div class="cart-product-price">
                  <span>
                    <i @click="deleteCartProduct(cart_detail)" class="fa fa-times-circle"></i>
                  </span>
                  <span>{{ parseFloat(cart_detail.total).toLocaleString(getLocale) }}</span>
                </div>
              </li>
            </div>
          </ul>
    ...
    <div class="cart-total-container">
              ...
              <tr>
                <td class="text-left" id="discname">
                  ...
                  <i
                    ...
                    @click="openDiscountModal('addDiscountToCart')"
                  ></i>
                </td>
      ...
      <div class="cart-button-container pos-action">
        <button
          type="button"
          class="btn btn-lg btn-pos-primary customer-btn"
          @click="changeCustomer"
        >
          <i class="fa fa-user-circle"></i>
          <span
            v-if="this.localObject.pos_cart_customer.name"
            :title="this.localObject.pos_cart_customer.name"
          >
            <div
              v-if="this.localObject.pos_cart_customer.name.length < 10"
            >{{ this.localObject.pos_cart_customer.name }}</div>
            <div v-else>{{ this.localObject.pos_cart_customer.name.substring(0,10)+".." }}</div>
          </span>
          <span v-else>{{ $t('pos_home.pos_cart.button_customer') }}</span>
          <i class="fa fa-pencil"></i>
        </button>
        <!-- <button type="button" class="btn btn-lg btn-pos-dark pay-btn" @click="payForCart"> -->
        <button
          type="button"
          class="btn btn-lg btn-pos-primary pay-btn"
          @click="payForCart"
        >{{ $t('pos_home.pos_cart.button_pay') }}</button>
        <button
          type="button"
          class="btn btn-lg btn-pos-primary hold-btn"
