<template>
    <div :class="{'vertical-content':!isMobileOnly}" v-if="isUserLogin">
        <div v-if="isMobileOnly" class="amount">
            <div class="control-group" >
                <label for="text_tendered" v-text="$t('pos_payment.pos_cash.text_tendered')"></label>
                <span class="control">{{ current_currency }} {{ parseFloat(tendered_amount).toLocaleString(getLocale) }}</span>
            </div>
            <div class="control-group" >
                <label for="text_tendered" v-text="$t('pos_payment.pos_cash.text_change')"></label>
                <span class="control control-disabled">{{ current_currency }} {{ parseFloat(change_amount).toLocaleString(getLocale) }}</span>
            </div>
        </div>
        <div class="pull-left total-details" v-else>
            <div class="payment-total">
                <span class="amount">{{ current_currency }} {{ parseFloat(grand_total).toLocaleString(getLocale) }}</span>
                <span class="text">{{ $t('pos_payment.pos_cash.text_total') }}</span>
            </div>
            <div class="payment-total">
                <span class="amount">{{ parseFloat(tendered_amount).toLocaleString(getLocale) }}</span>
                <span class="text">{{ $t('pos_payment.pos_cash.text_tendered') }}</span>
            </div>
            <div class="payment-total">
                <span class="amount">{{ current_currency }} {{ parseFloat(change_amount).toLocaleString(getLocale) }}</span>
                <span class="text">{{ $t('pos_payment.pos_cash.text_change') }}</span>
            </div>
        </div>
        <div :class="{'pull-right':!isMobileOnly}">
        ...
        <div class="text-right" v-if="!isMobileOnly">
        ...
        <div class="pos-action text-right" v-bind:style="{ 'padding-right': '0px' }">
            <button type="button" class="btn btn-lg btn-pos-primary" :class="{'action-btn-green': isMobileOnly}" @click="confirmOrder('confirmOrder')">
        ...
        methods: {
            ...
            validateOrder() {
                ...
                if (validate_status) {
                    ...
                    thisthis.final_order.tendered_amount = thisthis.tendered_amount;
                    thisthis.final_order.change_amount = thisthis.change_amount;
                    thisthis.final_order.payment_mode = 'cash';
