...
<template>
    <div class="pos-sales-main" :class="{'mobile': isMobileOnly}">
        <div class="pos-nav-container">
            <ul class="pos-nav-lists">
                <li v-for="(menu, index) in pos_product_menus" :key="index" :class="{ 'pos-nav': true }" :label="'menu_count_' + index">
                    <router-link class="nav-link" :to="{ path: menu.route }" replace>
                        {{ menu.name }}
                    </router-link>
                </li>
            </ul>
        ...
        <div class="pos-nav-content" v-bind:style="{'height': isMobileOnly ? 'calc(100% - 44px)' : container_height}">
            <router-view
