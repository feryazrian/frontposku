  ...
  <div class="product-lowstock-panel">
      ...
      <div class="pos-product-container" v-if="isUserLogin">
        ...
        >
          <div
            ...
            :key="product.id"
            @click="requestProductStock(product)"
          >
    ...
    <!-- <div class="nav-lowstock-request-list">
      <div class="container-panel-header">
        <h3>{{ $t("pos_product.menu_content.low_stock.text_request_product") }}</h3>
      </div>
      <div class="requested_list_container" v-bind:style="{'height': requested_list_height}">
        <ul class="requested_list" v-if="requested_count != 0">
          <li v-for="(request, index) in requested_list" v-bind:key="index">
            <div class="pull-left text-center">
              <div class="product-thumb">
                <img
                  v-if="request.base_image.small_image_url"
                  :src="request.base_image.small_image_url"
                />
                <label v-else class="product-placeholder"></label>
              </div>
              <div
                class="product-quantity"
              >{{ request.requested_quantity }} {{ $t("pos_product.menu_content.low_stock.text_qty") }}</div>
            </div>
        ...
        <div class="pos-action text-center">
          <button
            class="btn btn-md btn-pos-primary"
            type="submit"
            :text="$t('pos_product.menu_content.low_stock.button_send')"
            @click="sendRequest()"
          >{{ $t('pos_product.menu_content.low_stock.button_send') }}</button>
        </div>
      </div>
    </div>-->
...
export default {
  ...
  methods: {
      ...
      if (this.pos_offline) {
        if (Object.keys(this.localObject.pos_lowstock_products).length > 0) {
          this.total_product = Object.keys(
            this.localObject.pos_lowstock_products
          ).length;
    ...
    getRemainingQty(product) {
      ...
      if (product.type == "simple") {
        ...
        if (Object.keys(thisthis.localObject.pos_offline_orders).length > 0) {
          // console.log("pos_offline_orders", thisthis.localObject.pos_offline_orders)
    ...
    //   } else {
    //     this.$http
    //       .get("/api/pos/auth/getLowRequestedProducts", {
    //         params: {
    ...
    //     if (this.localObject.pos_cashier.id) {
    //       this.$http
    //         .post("/api/pos/auth/deleteRequest", {
